...
| `/goal <objective>`, `/goal --research <objective>`, `/goal --simple <objective>`, `/goal status`, `/goal pause`, `/goal resume`, `/goal clear` | Starts, checks, pauses, resumes, or clears Goal | Goal is not in any keyboard cycle; clearly long-horizon goals automatically enable AutoResearch after Goal is explicitly started. Goal automatically selects a simple, write, or research turn budget. |
...
| `/loopinstant <interval> <prompt>` | Runs a prompt immediately, then on a schedule | `/loopinstant 2m check the deploy` fires immediately, then every 2 minutes; `/loopinstant check the deploy` fires immediately then dynamic; Prefix with `--forever` for endless loop (no 7-day expiry). |
