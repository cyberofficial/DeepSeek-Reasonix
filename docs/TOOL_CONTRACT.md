...
| `cron_create_action` | false | Create a scheduled task that runs a host command every interval. Set match_pattern (regex) to only invoke the LLM when output matches; set action_response=false for silent checks. Set expires_in (e.g. 5m/2h/1d) to self-delete the task after that long. The command may emit 'ai-response: true' or 'ai-response: false' on stdout to force or suppress the LLM wake. Use with /loopaction or cron_list/cron_delete. |
...
| `trigger_delay` | false | Create a one-shot countdown trigger that fires once after a delay (e.g. "check in 30s"). With prompt, the LLM receives the prompt at fire time; with command, the host runs the command and the LLM is woken only if the output matches match_pattern (or action_response=true). The task deletes itself after firing; the LLM can continue it with schedule_wakeup. Use with cron_list/cron_delete. |
