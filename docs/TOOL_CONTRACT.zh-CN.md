...
| `cron_create_action` | false | 创建按固定间隔运行主机命令的排定任务。设置 match_pattern（正则）可仅在输出匹配时唤起 LLM；设置 action_response=false 表示静默检查。设置 expires_in（如 5m/2h/1d）可在指定时长后自动删除任务。命令可在 stdout 输出 'ai-response: true' 或 'ai-response: false' 来强制或抑制 LLM 唤起。可与 /loopaction 或 cron_list/cron_delete 配合使用。 |
