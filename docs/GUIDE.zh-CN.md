...
| `Ctrl+B` | 展开或收起较长 shell 输出 | 较长 shell 输出的提示行也可点击；全屏 TUI 开启鼠标接管时，文本选区由应用内处理。 |
| `/goal <目标>`、`/goal --research <目标>`、`/goal --simple <目标>`、`/goal status`、`/goal pause`、`/goal resume`、`/goal clear` | 启动、查看、暂停、恢复或清除 Goal | Goal 不在任何键盘循环中；明确长期目标在 Goal 显式启动后自动启用 AutoResearch。Goal 自动选择简单、写入或研究轮次预算。 |
| `/loop [间隔] [提示词]` | 循环运行提示词，或管理计划循环 | `/loop 5m 检查部署` 每 5 分钟运行一次提示词；`/loop <提示词>` 运行动态循环，由代理通过 `schedule_wakeup` 选择每次延迟；裸 `/loop` 使用 `loop.md` 或内置维护提示词。前缀 `--forever`（如 `/loop --forever 5m 检查部署`）创建永不过期循环（默认：任务 7 天后过期）。落在回合中的触发会将提示词作为标记的引导消息（`⏰ scheduled task <id>:`）注入运行回合，而非中断；空闲触发作为完整回合运行。任务按工作目录持久化在 `<workspace>/.reasonix/scheduled-tasks.json` 中——一个文件由该文件夹启动的所有聊天共享，经受 `/new` 和 `/clear`。固定循环通过请求代理取消（`cron_delete`）；动态循环在 `Esc` 暂停，第二次 `Esc` 取消。状态栏显示下一次计划触发为 `NEXT JOB <id> <时间>`。 |
| `/loopstatus [on|off|auto]` | 切换 `NEXT JOB` 状态栏指示器 | 默认 `auto`：仅在有待定计划任务时显示。`on` 总显示（无任务时显示 `none`）；`off` 即使任务运行中也隐藏。裸 `/loopstatus` 报告当前模式。 |
| `/looplist` | 本地列出工作目录的计划任务（无模型调用） | 显示每个任务的 ID、计划（cron 或动态）、下次触发、一次性/无过期标记。 |
| `/loopdel <id>` | 本地按 ID 删除计划任务（无模型调用） | `/loopdel 3f9a2c11` 取消该任务；确认会报告成功或任务不存在。 |
| `/loopdelay [--action] <时长> <提示词或命令>` | 创建一次性倒计时触发，延迟后仅触发一次 | `/loopdelay 2m 检查上游提交并汇报` 2 分钟后触发一次提示词并删除任务；`/loopdelay --action 2m --match UPSTREAM-HAS-NEW bash check.sh` 2 分钟后运行命令一次，仅当输出匹配正则时唤醒 LLM。时长接受 Go 格式（`2m`、`90s`、`1h30m`）或自然语言（`5 分钟`）。触发后 LLM 可调用 `schedule_wakeup` 继续作为动态循环，或让其结束。 |
| `/loopinstant <间隔> <提示词>` | 立即运行提示词，然后按计划重复 | `/loopinstant 2m 检查部署` 立即执行，随后每 2 分钟运行一次；`/loopinstant 检查部署` 立即执行后转为动态循环；前缀 `--forever` 创建永不过期循环（默认任务 7 天后过期）。 |
