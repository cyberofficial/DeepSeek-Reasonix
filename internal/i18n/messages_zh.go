...
var Chinese = Messages{
	...
	ChatStatusCycleHint:                    "Shift+Tab 循环询问/自动/计划/分推理 · Ctrl+Y YOLO",
	ChatStatusCycleHintCompact:             "Shift+Tab 询问/自动/计划/分推理 · Ctrl+Y YOLO",
	ChatStatusSplitReasonHint:              "分推理：主模型规划，从模型执行",
	...
	ApprovalToolLabelCommandTask:           "定时主机命令",
	...
	WriteAccessApprovalChoices:             "1. 允许一次\n2. 本会话允许\n3. 保存到项目配置\n4. 拒绝\n选择 [1/2/3/4]（兼容 y/a/p/n）",
	WriteAccessHomeWarning:                 "此授权包含用户主目录下的目录。请谨慎操作——此处的更改可能影响整个系统。",
	WriteAccessMergedPermissionHint:        "部分目录可能仍需普通权限批准，此外此授权将生效。",
	WriteAccessProjectHint:                 "此访问权限将保存到项目的 reasonix.toml，供后续会话使用。",
	CommandTaskSubjectPrefix:               "定时执行主机命令：",
	CommandTaskReason:                      "这将注册一个定时执行的主机命令，之后会在每次触发时无人值守地运行。它不受沙箱保护，并使用当前会话的环境，因此需要你明确批准。",
	CommandTaskDeclined:                    "用户拒绝了该定时主机命令——不要重试；请询问用户希望如何处理。",
	...
	CmdLoopAction:       "按计划运行主机命令，并在条件匹配时触发 LLM",
	CmdLoopDelay:        "创建一个一次性倒计时触发器，在延迟后触发提示词（或配合 --action 触发主机命令）",
	CmdLoopDelayHint:    "用法: /loopdelay [--action] <时长> <提示词|命令> [--match <正则>] [--no-ai]，例如 /loopdelay 2m 检查上游提交",
